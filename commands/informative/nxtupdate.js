...
module.exports = {
    ...
    async run(client, message, args) {
        try {
            ...
            message.reply(`${Math.floor(i.length('days'))} days until the next update (${client.data.update_day})`).then(msg => { setTimeout(() => msg.delete(), client.configuration.cmd.timeout); })
        } catch (error) {
