    ...
    async run(client, message, args) {
        try {
            ...
            getBddconnection().then((connection) => {
                connection.connect()
                connection.query(`SELECT DISTINCT heroes.name,ex_weapon.name as 'ex_weapon',ex_weapon.link as 'ex_weapon_link',ex_weapon.emote_type as 'ex_weapon_type',hero_pic,hero_type.type,hero_type.hexcode AS 'type_hexcode',hero_role.role,hero_role.emote AS 'role_emote',weapon,pp_link,champion_link from heroes LEFT JOIN ex_weapon ON heroes.weapon=ex_weapon.id LEFT JOIN hero_type ON heroes.type = hero_type.id LEFT JOIN hero_role ON heroes.role = hero_role.id WHERE heroes.name LIKE '%${args.join(' ')}%'`,
                    function (error, results, fields) {
                        ...
                        if (!results[0]) {
                            ...
                            message.reply({ embeds: [emb] }).then(msg => { setTimeout(() => msg.delete(), 10000); })
                        }
                    });
            })
